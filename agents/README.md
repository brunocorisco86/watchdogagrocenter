...
* **Objetivo**: Focar na integração do sistema com o Alpine Linux do Raspberry Pi 3B. Garantir a eficiência máxima de memória (minimalista) e o correto agendamento no crontab do sistema.
* **Prompt Base**:
  ```text
...
* **Objetivo**: Garantir que as atualizações do ambiente de desenvolvimento local cheguem ao Raspberry Pi de produção na LAN via Git pull/testing/deploy.
* **Prompt Base**:
  ```text
  Você é o DevOps / Especialista de CI/CD para o projeto Watchdog.
  Sua tarefa é analisar como gerenciar deploys locais no Raspberry Pi 3B na rede local (LAN).
